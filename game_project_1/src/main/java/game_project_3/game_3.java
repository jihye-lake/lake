...
//   Thread th2;   // 스레드 생성
   ...
   game_Frame() {   // 프레임 생성
      ...
      setTitle("내가 제일 맛있는 과자다!!!!!!!!! 다 비켜!!!!!!!!");      // 프레임 이름 설정
   ...
   public void init() {
      ...
      me_img = tk.getImage("./src/main/img/popcorn11.png");
      Missile_img = tk.getImage("./src/main/img/popcorn_2.png");
      Enemy_img = tk.getImage("./src/main/img/snack_bite.png");
      
      e_w = ImageWidthValue("./src/main/img/snack_bite.png");
      e_h = ImageHeightValue("./src/main/img/snack_bite.png");
      
      m_w = ImageWidthValue("./src/main/img/popcorn_2.png");
      m_h = ImageHeightValue("./src/main/img/popcorn_2.png");
   }
...
//      th2.start();                  // 스레드 실행
   ...
   public void MissileProcess() {   // 미사일 처리 메소드
      if(KeySpace) {   // 스페이스바 키 상태가 true 면
         ms = new Missile(x, y);      // 미사일 발사 위치를 제대로 하기 위한 좌표조정
...
//         ms = new Missile(x,y);      // 좌표 체크해서 넘기기
   ...
   public void EnemyProcess() {   // 적 행동 처리 메소드
      ...
      if (cnt % 150 == 0) {      // 루프 카운트 300회 마다
         en = new Enemy(f_height - 600, 0);
         Enemy_List.add(en);   // 각 좌표로 적을 생산한 후 배열에 추가한다
         en = new Enemy(f_height - 400, 0);
         Enemy_List.add(en);
         en = new Enemy(f_height - 200, 0);
         Enemy_List.add(en);
/*         
...
//         buffg.drawImage(Missile_img, ms.pos.x + 250, ms.pos.y + 55, this);
         buffg.drawImage(Missile_img, ms.x + 105, ms.y - 60, this);   // 변수명 변경으로 위에서 아래로 수정 됨
...
*/         
   ...
   public void Draw_Enemy() {   // 적 이미지를 그리는 부분
      for (int i = 0; i < Enemy_List.size(); ++i) {     
         ...
         buffg.drawImage(Enemy_img, en.x + 100, en.y - 50, this);   // 배열에 생성된 각 적을 판별하여 이미지 그리기
      }
